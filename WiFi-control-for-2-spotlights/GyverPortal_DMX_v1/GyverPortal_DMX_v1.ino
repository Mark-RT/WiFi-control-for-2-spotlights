...
void build() {
  ...
  M_BOX(GP.BUTTON_MINI("Button_1_light", "1", "", GP_GREEN, "", 0, 1);
        ...
        GP.LABEL_BLOCK("  ");
        GP.BUTTON_MINI("Palitra_button", "Палитра", "", GP_ORANGE, "", 0, 1);
        GP.LED_GREEN("Palitra_light", flagPalitra);
       );
  ...
  M_BLOCK_THIN(
    ...
    M_BOX(GP.HR(); );
    M_BOX(GP.LABEL("Палитра"); GP.BREAK(); );
    M_BOX(GP.SLIDER_C("Palitra_slider", palitra_value, min_val, 1530, 1, 0, GP_ORANGE); );            // функциональный выбор
    M_BOX(GP.HR(); );
    M_BOX(GP.LABEL("Красный"); GP.BREAK(); );
    M_BOX(GP.SLIDER_C("Red_slider", red_brightness, min_val, max_val, step_slider, 0, GP_RED_B); );           // яркость R
    M_BOX(GP.HR(); );
    M_BOX(GP.LABEL("Зелёный"); GP.BREAK(); );
    M_BOX(GP.SLIDER_C("Green_slider", green_brightness, min_val, max_val, step_slider, 0, GP_GREEN_B); );       // яркость G
    M_BOX(GP.HR(); );
    M_BOX(GP.LABEL("Синий"); GP.BREAK(); );
    M_BOX(GP.SLIDER_C("Blue_slider", blue_brightness, min_val, max_val, step_slider, 0, GP_BLUE_B); );         // яркость B
    M_BOX(GP.HR(); );
...
void action() {
  if (ui.click()) {
    if (ui.click("Button_1_light")) {
    ...
    else if (ui.click("Palitra_button")) {
      flagPalitra = !flagPalitra;
      if (flagPalitra = 1) colorWheel(palitra_value);
      flag_strobe = 1;
      flagRainbowLed = 1;
      red_brightness = 0;
      green_brightness = 0;
      ...
      flagRainbowLed = 1;
      flagPalitra = 0;
      red_brightness = max_val;
      green_brightness = max_val;
      blue_brightness = max_val;
    ...
    else if (ui.clickInt("Palitra_slider", palitra_value)) {
      if (flagPalitra = 1) {
        colorWheel(palitra_value);
      }
    }

    else if (ui.clickInt("Red_slider", red_brightness)) {
      for (int i = 2 + changer1; i <= lights_pcs * 8; i = i + 8 + changer2) {
        dmx.write(i, red_brightness);
      }
    }
    else if (ui.clickInt("Green_slider", green_brightness)) {
      for (int i = 3 + changer1; i <= lights_pcs * 8; i = i + 8 + changer2) {
        dmx.write(i, green_brightness);
      }
    }
    else if (ui.clickInt("Blue_slider", blue_brightness)) {
      for (int i = 4 + changer1; i <= lights_pcs * 8; i = i + 8 + changer2) {
        dmx.write(i, blue_brightness);
      }
    }
